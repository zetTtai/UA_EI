...
// Ruta para la información de indice almacenada en disco
#define PATH1 "indiceDisco.txt"
// Ruta para la información de indiceDocs almacenada en disco
...
{
    ofstream file(PATH1, ios::app); //Añadimos ios::app para poder añadir lineas al final del fichero
    if(file)
    {
        //    \t      Comienza  list      \t                    
            ...
            if(count == line)
            {
                if(path == PATH1) //indice
                {
                    if(!newline.empty())
                    {
                        ...
                        temp << ""; // Si newline está vacio entonces elimina
                }
                else // indiceDocs sólo se llama a esta función cuando hay que eliminar la información de un documento anticuado 
                {
                    temp << "";
                }
            }
...
unordered_map<string, list<int>>
IndexadorHash::ExtractInfo(string& str, InfDoc& doc) const
{
    unordered_map<string, list<int>> infTerm;
    int pal, sinparada;
    pal= sinparada= 0;
    stemmerPorter stem;
...
bool
IndexadorHash::AddTerms(const string& ruta, const unordered_map<string, list<int>>& infTerm, InfDoc& inf)
{
    if(!ruta.empty()) // Es un documento
    {
        try
        {
            if(!this->almacenarEnDisco)// TODO probar si recorriendo el índice es mejor que recorrer todas las palabras del fichero
            {
                ...
                this->indiceDocs[ruta]= inf; // Añadimos el documento con toda su informaci?n a indiceDocs
        ...
        catch(const std::bad_alloc& e)
        {
            if(!this->almacenarEnDisco)
            {
                ...
                cerr << "Último documento indexado: " << ruta << '\t' << "Último término indexado: " << this->indice.end()->first << '\n';
            }
            else
            {
                cerr << "ERROR: (Indexar) Se ha alcanzado el limite de memoria.\n";
                cerr << "Último documento indexado: " << ruta << '\t' << "Último término indexado: " << this->indiceDisco.end()->first << '\n';
            }
            return false;
        }
    }
                        ...
                        if(fecha.moreRecentThan(auxDoc.getFecha())) // El nuevo es mas reciente que el ya indexado
                        {
                            ...
                            for(auto& termino : indiceDocsDisco)
                                if(termino.second > this->indiceDocsDisco[ruta])
                                    this->indiceDocsDisco[termino.first] -= 1; // Tenemos que restar uno a la linea en la que se encuentra la información
...
IndexadorHash::ImportarIndices(const string& ruta)
{
    ...
    if(fEntrada)
    {
        ...
        while(linea != "*") // indice
        {
            ...
            if(!this->almacenarEnDisco)
                this->indice.insert({termino, inf}); //Añadimos el termino al indice
            else
...
IndexadorHash::Devuelve(const string& word, InformacionTermino& inf) const
{
    ...
    if(!this->almacenarEnDisco)
    {
        ...
        {
            inf= got->second; //Guardamos la informacion del termino deseado en inf
            return true;
        }
    }
    else
    {
        ...
        {
            string str= GetInfo(PATH1, this->indiceDisco.at(aux)); // Obtenemos la linea con la información
            inf= GetInfTerm(str, 0);
            return true;
        }
    }
        ...
        if(got != this->indiceDisco.end())
        {
            ...
            for(auto& termino : indiceDisco) // TODO Probar
                if(termino.second > this->indiceDisco.at(aux))
                    this->indiceDisco[termino.first] -= 1; // Tenemos que restar uno a la linea en la que se encuentra la información
                ...
                for(auto& doc : inf.getDocs())
                {
                    if(doc.first == infDoc.getId())
                    {
                        ...
                        if(!inf.getDocs().empty()) // Como ft != 0, hay que reescribir la informacion en el documento con la nueva
                            AddInfTerm(inf, aux); // Añade y modifica el long int asociado a la linea del termino aux
                        else
                            this->indiceDisco.erase(aux);
                    }
                }
...
IndexadorHash::Actualiza(const string& word, const InformacionTermino& inf)
{
    ...
    else
    {
        ...
        if(got != this->indiceDisco.end())
        {
            ModifyLine(this->indiceDisco.at(aux), 0, "", PATH1); //Eliminamos la información antigua
            for(auto& termino : indiceDisco) // TODO Probar
                if(termino.second > this->indiceDisco.at(aux))
                    this->indiceDisco[termino.first] -= 1; // Tenemos que restar uno a la linea en la que se encuentra la información
            AddInfTerm(inf, aux);
