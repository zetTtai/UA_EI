...
InfTermDoc::InfTermDoc(const int& ft, const vector<int>& posTerm)
{
    this->ft= ft;
...
void 
InfTermDoc::setPosTerm(const vector<int>& posTerm)
{
    this->posTerm= posTerm;
    // this->posTerm.sort(std::less<int>()); // De menor a mayor SIEMPRE está ordenado
}
...
InformacionTerminoPregunta::InformacionTerminoPregunta (const int& ft, const vector<int>& posTerm)
{
    this->ft= ft;
    this->posTerm= posTerm;
}
